...
def clip_replace_constant(y, yhat, out_variables):
    ...
    # Clamp precipitation to non-negative if present in output variables
    ...
    # Clamp composite reflectivity to non-negative if present (dBZ >= 0)
    if "composite_reflectivity" in out_variables:
        refl_index = out_variables.index("composite_reflectivity")
        torch.clamp_(yhat[:, refl_index, :, :], min=0.0)
...
def create_data_module(data_key, config, world_rank, device, do_tiling, div, overlap):
    ...
    if do_tiling:
        ...
        superres_mag = config["model"].get("superres_mag", 4)
        lat, lon = data_module.get_lat_lon()
        yout = len(lat) // div
        yinp = yout // superres_mag + overlap
...
def main(device):
    ...
    # Count constant variables present in input vars for the residual skip path
    constant_vars = ["land_sea_mask", "orography", "lattitude", "landcover"]
    num_constant_vars = sum(1 for var in constant_vars if var in default_vars)
    ...
    model_kwargs = {
        ...
        "input_refine_cnn": conf["model"].get("input_refine_cnn", False),
    }

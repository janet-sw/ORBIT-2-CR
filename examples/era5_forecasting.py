...
def clip_replace_constant(y, yhat, out_variables):
    ...
    # NOTE: Do NOT clamp composite_reflectivity in normalized space.
    # The normalization maps 0 dBZ to ~-0.3, so clamping at 0 in normalized
    # space kills gradients and prevents learning. Apply physical constraints
    # only after denormalization (e.g., during inference/evaluation).
...
def training_step(
    ...
    # Debug: print shapes on first batch only
    if batch_idx == 0:
        print(f"[DEBUG] x shape (input):  {x.shape}  | in_variables: {in_variables}", flush=True)
        print(f"[DEBUG] y shape (target): {y.shape}  | out_variables: {out_variables}", flush=True)
    ...
    if batch_idx == 0:
        print(f"[DIAG] yhat: mean={yhat.mean().item():.4f} std={yhat.std().item():.4f} min={yhat.min().item():.4f} max={yhat.max().item():.4f}", flush=True)
        print(f"[DIAG] y:    mean={y.mean().item():.4f} std={y.std().item():.4f} min={y.min().item():.4f} max={y.max().item():.4f}", flush=True)
        print(f"[DIAG] x:    mean={x.mean().item():.4f} std={x.std().item():.4f} min={x.min().item():.4f} max={x.max().item():.4f}", flush=True)
    ...
    losses = train_loss_metric(yhat, y_matched)
    ...
    # Optional: image gradient loss to encourage sharp spatial features
    if grad_loss_weight > 0.0:
        from climate_learn.metrics.functional import image_gradient_fn
        grad_loss = image_gradient_fn(yhat, y_matched)
        loss = loss + grad_loss_weight * grad_loss
        if batch_idx == 0:
            print(f"[DIAG] lat_mse={losses[-1].item() if losses.dim() > 0 else losses.item():.4f} grad_loss={grad_loss.item():.4f} total={loss.item():.4f}", flush=True)
...
def create_model_and_losses(config, device, world_rank, in_vars, out_vars, data_module):
    ...
    num_gpus = int(os.environ.get("SLURM_NTASKS", config["trainer"]["num_gpus"]))
...
def create_losses_after_fsdp(config, device, world_rank, model, data_module, in_vars, out_vars):
    ...
    # Load training loss from config (default: lat_mse)
    train_loss_name = config["trainer"].get("train_loss", "lat_mse")
    if world_rank == 0:
        print(f"Using {train_loss_name} for training loss", flush=True)
    train_loss = cl.load_loss(device, model, train_loss_name, True, train_metainfo)
...
def create_data_module(config, world_rank, device):
    ...
    Supports two modes:
    - Forecasting: single data directory (era5_dir), with GPU-based downsampling
    - Downscaling: separate low_res_dir / high_res_dir (e.g. HRRR 0.1° -> 0.05°)

    Returns:
        tuple: (data_module, train_dataloader, val_dataloader, in_vars, out_vars)
    """
    forecast_type = config["data"].get("forecast_type", "downscaling")

    # Define variables - expand pressure level vars
    ...
    if world_rank == 0:
        print(f"Creating data module for {forecast_type}", flush=True)
    ...
    if forecast_type == "downscaling":
        # Downscaling mode: separate input/output directories at different resolutions
        ...
        # No GPU-based downsampling needed for downscaling mode
        data_module.resize_config = None

    elif forecast_type in ("direct", "iterative"):
    ...
    # GPU-based downsampling config (only for forecasting modes, not downscaling)
    if forecast_type != "downscaling":
...
def train_epoch(
    ...
    for batch_idx, batch in enumerate(train_dataloader):
        ...
        loss = training_step(batch, batch_idx, model, device, var_weights, train_loss, resize_config, grad_loss_weight=grad_loss_weight)
        ...
        if world_rank == 0 and batch_idx % log_interval == 0:
            ...
            # Diagnostic: gradient norms and model output stats
            grad_norm = 0.0
            grad_count = 0
            zero_grad_params = 0
            ...
            print(
                ...
                f"ZeroGrads: {zero_grad_params}/{grad_count}",
                flush=True
            )
...
def main(device):
    ...
    # Image gradient loss weight (0.0 = disabled)
    grad_loss_weight = float(config["model"].get("grad_loss_weight", 0.0))
    if world_rank == 0 and grad_loss_weight > 0.0:
        print(f"Image gradient loss enabled with weight={grad_loss_weight}", flush=True)

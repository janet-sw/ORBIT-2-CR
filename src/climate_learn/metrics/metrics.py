...
class LatWeightedMAE(LatitudeWeightedMetric):
    """Computes latitude-weighted mean absolute error (L1).

    L1 loss produces sharper predictions than L2 (MSE) because it penalizes
    errors linearly rather than quadratically, so the model doesn't hedge
    as aggressively toward the mean for rare extreme values.
    """
    ...
    def __call__(
        ...
        super().cast_to_device(pred)
        return mae(pred, target, aggregate_only=self.aggregate_only, lat_weights=self.lat_weights)
